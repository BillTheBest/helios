 ...
 * <p><code>org.helios.helpers.RuntimeMBeanServerConnection</code></p>
 */
...
public class RuntimeMBeanServerConnection implements MBeanServerConnection {
	...
	/** A map of RuntimeMBeanServerConnections keyed by their delegate MBeanServerConnections */
	protected static final Map<MBeanServerConnection, RuntimeMBeanServerConnection> instances = new ConcurrentHashMap<MBeanServerConnection, RuntimeMBeanServerConnection>();
	...
	public static RuntimeMBeanServerConnection getInstance(MBeanServerConnection innerConnection) {
		if(innerConnection==null) throw new IllegalArgumentException("The passed delegate MBeanServerConnection was null", new Throwable());
		RuntimeMBeanServerConnection rserver = instances.get(innerConnection);
		if(rserver==null) {
			synchronized(instances) {
				rserver = instances.get(innerConnection);
				if(rserver==null) {
					rserver = new RuntimeMBeanServerConnection(innerConnection);
					instances.put(innerConnection, rserver);
				}
			}
		}
		return rserver;
