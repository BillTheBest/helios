 ...
 * <p><code>org.helios.helpers.RuntimeMBeanServer</code></p>
 */
...
public class RuntimeMBeanServer extends RuntimeMBeanServerConnection implements MBeanServer {
	...
	public static RuntimeMBeanServer getInstance(MBeanServer innerServer) {
		if(innerServer==null) throw new IllegalArgumentException("The passed delegate MBeanServer was null", new Throwable());
		RuntimeMBeanServer rserver = (RuntimeMBeanServer) instances.get(innerServer);
		if(rserver==null) {
			synchronized(instances) {
				rserver = (RuntimeMBeanServer) instances.get(innerServer);
				if(rserver==null) {
					rserver = new RuntimeMBeanServer(innerServer);
					instances.put(innerServer, rserver);
				}
			}
		}
		return rserver;
