 ...
 *
 */
package org.helios.collectors.ping;
...
public class PingCollector extends AbstractCollector {
	...
	public CollectionResult collectCallback() {
		...
		result.setResultForLastCollection(CollectionResult.Result.SUCCESSFUL);
			...
			if(logErrors)
				log.error("An error occured during collect callback for PingCollector: "+this.getBeanName(),ex);
	...
	public String ping(String ipAddress) throws Exception{
		...
		try {
			...
			log.info("ping took: "+ (System.currentTimeMillis() - start));
			return buff.toString();
		}finally {
			try { reader.close(); } catch (Exception e) {log.debug(e.getMessage());}
		}
	}	
				...
				if(line.startsWith("Minimum")) {
					...
					for(String reading: readings) {
						try {
							...
							tracer.trace(value, timeType, StringHelper.append(tracingNameSpace, true, hostName));
						} catch (Exception e) {log.debug(e.getMessage());}						
					}
				} else if(line.startsWith("Packets:")) {
					try {
						int value = Integer.parseInt(line.split("\\(")[1].split("\\)")[0].split("%")[0]);
						tracer.trace(value, "Packet Loss", StringHelper.append(tracingNameSpace, true, hostName));
						if(value==100) {
							tracer.trace(-1, "Minimum", StringHelper.append(tracingNameSpace, true, hostName));
							tracer.trace(-1, "Maximum", StringHelper.append(tracingNameSpace, true, hostName));
							tracer.trace(-1, "Average", StringHelper.append(tracingNameSpace, true, hostName));
						}
					} catch (Exception e) {}
				}			
			}
			...
			while((line=br.readLine())!=null) {
				...
				if(line.startsWith("rtt")) {
					...
					try {
						...
						tracer.trace(value, "Minimum", StringHelper.append(tracingNameSpace, true, hostName));										
					} catch (Exception e)  {log.debug(e.getMessage());}
					try {
						value = (long)Float.parseFloat(values[1]);
						tracer.trace(value, "Average", StringHelper.append(tracingNameSpace, true, hostName));								
					} catch (Exception e)  {}		
					try {
						value = (long)Float.parseFloat(values[2]);
						tracer.trace(value, "Maximum", StringHelper.append(tracingNameSpace, true, hostName));								
					} catch (Exception e)  {}		
	 ...
	 */
	@JMXOperation(name="reportServers", description="Display a list of servers monitored by this collector.")
