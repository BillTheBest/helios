 ...
 *
 */
package org.helios.collectors.webservice;
...
public class WebServiceCollector extends URLCollector {
	...
	public void postStart() throws CollectorStartException{
		...
		if(getWsStyle()!=null && getWsStyle().equalsIgnoreCase("SOAP")){
			if(postMethod!=null){
				...
				throw new CollectorStartException("HTTP Post method is not initialized properly for collector bean: "+this.getBeanName());
			}
		}
	}
	 ...
	 */
	@JMXAttribute (name="CollectorVersion", description="Version used for Web Service collector", mutability=AttributeMutabilityOption.READ_ONLY)
	public String getCollectorVersion() {
		return "WebServiceCollector v. "+ WEBSERVICE_COLLECTOR_VERSION;
	}
	
	@JMXAttribute (name="RequestXML", description="XML Payload to be sent to the server", mutability=AttributeMutabilityOption.READ_ONLY)
